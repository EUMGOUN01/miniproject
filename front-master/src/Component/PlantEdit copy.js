...
const PlantEdit = () => {
  ...
  const [category, setCategory] = useState(''); // 게시글 카테고리 상태
  ...
  useEffect(() => {
    const fetchPost = async () => {
      try {
        ...
        //이미지 데이터를 처리
        if (data.simges && data.simges.length > 0) {
          console.log('파일데이터', data);
          const fileData = data.simges.map(data => ({
  ...
  const handleRemoveFile = (file) => {
    if (file.existing) {
      setRemovedFiles(files => [...files, file.simgid]); // 삭제된 기존 파일 ID 저장
    }
